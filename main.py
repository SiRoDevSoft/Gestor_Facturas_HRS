...
#     # auth.registrar_usuario("admin", "Stock5600", "Color?", "Azul")
...
def main():
    ...
    # Inicializamos el gestor de seguridad ADENTRO del main
    auth = AuthManager()

    # Si NO está autenticado, renderizamos login y DETENEMOS la ejecución
    if not st.session_state.autenticado:
        render_login(auth)
        st.stop() # <--- Evita que el resto del script intente cargar y cause parpadeo

    # --- SIDEBAR NAVEGACIÓN (Solo visible si pasó el stop) ---
    with st.sidebar:
        if st.button("<< Cerrar Sesión", type="secondary"):
            st.session_state.autenticado = False
            st.rerun()
    ...
    vistas = {
        ...
        "Consultas Históricas": render_consultas,
        "Pronostico Empresa": render_dashboard,
        "Configuración": lambda: render_configuracion(auth) # Pasamos auth como parámetro
    }
    
    # Ejecutamos la vista seleccionada
    vistas[menu]()
