...
# #     st.header("⚙️ Gestión de Flota e Integridad de Datos")
...
# #     with col_m:
# #         st.session_state.config_lucro = st.number_input(
# #             "Margen de Recargo General (%)", 
# #             min_value=0.0, 
# #             value=st.session_state.config_lucro,
# #             help="Este porcentaje se aplicará al costo de Movistar en los boletos de cobro."
# #         )
# #     st.divider()

# #     # --- SECCIÓN B: GESTIÓN DE LÍNEAS (Tu código) ---
# #     tab1, tab2 = st.tabs(["📝 Editor Maestro", "📋 Lista Completa"])

# #     with tab1:
# #         opciones = ["+ NUEVA LÍNEA"] + [f"{k} - {v['nombre']}" for k, v in config.items()]
...
# #             l_sug, n_sug, c_sug, edit_mode = "", "", "NINGUNO", False
# #         else:
# #             l_sug = seleccion.split(" - ")[0]
# #             n_sug = config[l_sug]["nombre"]
# #             c_sug = config[l_sug].get("grupo", "NINGUNO") # Ajustado a 'grupo' para consistencia
# #             edit_mode = True
...
# #         with col2:
# #             cats = ["NINGUNO", "VENTAS", "PATIO", "REPARTO", "ADMINISTRACION", "TERCEROS", "GERENCIA", "BAJA"]
# #             idx = cats.index(c_sug) if c_sug in cats else 0
# #             cat_f = st.selectbox("Asignar a Grupo", cats, index=idx)

# #         if st.button("💾 GUARDAR CAMBIOS", type="primary"):
# #             if linea_f and nombre_f and cat_f != "NINGUNO":
# #                 config[linea_f] = {"nombre": nombre_f.upper().strip(), "grupo": cat_f}
...
# #         datos_tabla = [{"Línea": k, "Nombre": v["nombre"], "Grupo": v.get("grupo", "S/D")} for k, v in config.items()]
...
#         except FileNotFoundError:
#             return {}
#     # Función para guardar el margen
...
#         # Listados Definidos
#         GRUPOS_VALIDOS = ["EMPRESA", "KOPRAM", "LUCARELLI", "CORREA", "PETISO", "PERRET", "TERCEROS_HRS","PIEDECASAS"]
#         CATEGORIAS_VALIDAS = ["VENTAS", "GERENCIA", "PATIO", "ADMINISTRACION", "REPARTO", "EXTERNOS", "BAJA"]
...
#             with col2:
#                 # Selector de Grupo (Para Boletos)
...
#                 # Selector de Categoría (Para Análisis Interno)
...
#                     # Guardamos ambas clasificaciones
#                     config[linea_f] = {
